...
This is disgolink v3.1.0 with one fix: `Rotation.RotationHz` typed as
`float64` instead of `int`. Lavalink v4 both accepts and echoes fractional
rotation frequencies (e.g. the classic 0.2 Hz "8D" sweep), so the stock
`int` type breaks response parsing whenever a rotation filter is active.
Wired in via the `replace` directive in the root go.mod. Drop this fork
when upstream fixes the type.
